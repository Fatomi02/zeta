<template>
    ...
    <transition name="fade-left">
        <div v-if="isOpen" @click="toggleMenu" class="modal_overlay lg:hidden">
            <div @click.stop.prevent class="h-screen relative w-[78%] bg-navy-blue flex flex-col justify-between pb-10">
                <div class="p-6 flex flex-col gap-10">
                    <h1 class="text-4xl text-center text-white font-extrabold">FinCore</h1>
                    <div class="flex gap-4 items-center">
                        <div class="w-[60px] h-[60px] rounded-full text-xl bg-light-blue flex justify-center items-center">
                            {{ user?.name ? user?.name.slice(0, 1) : 'U' }}
                        </div>
                        <div class="flex flex-col gap-1">
                            <h2 class="font-medium text-white capitalize"> {{ user?.name ? user?.name : 'Unknown User'
                                }}</h2>
